...
const PolaroidCard: React.FC<PolaroidCardProps> = ({ imageUrl, caption, status, error, memory, dragConstraintsRef, onShake, onDownload, isMobile, disableAnimation = false, imageFilter }) => {
    ...
    useEffect(() => {
        if (disableAnimation) {
            if (!isDeveloped) setIsDeveloped(true);
            return;
        }
        if (status === 'pending') {
    ...
    }, [imageUrl, status, disableAnimation, isDeveloped]);

    // When the image is loaded, start the developing animation.
    useEffect(() => {
        if (isImageLoaded && !disableAnimation) {
            const timer = setTimeout(() => {
                setIsDeveloped(true);
            }, 200); // Short delay before animation starts
            return () => clearTimeout(timer);
        }
    }, [isImageLoaded, disableAnimation]);
    ...
    const developingFilter = 'sepia(1) contrast(0.8) brightness(0.8)';
    const finalFilter = imageFilter || 'none';
    ...
    const cardInnerContent = (
        <>
                ...
                {status === 'done' && imageUrl && (
                    <>
                        ...
                        <img
                            ...
                            style={{
                                filter: isDeveloped ? finalFilter : developingFilter,
                                opacity: isImageLoaded ? (isDeveloped ? 1 : 0.8) : 0,
                            }}
                        />
                    </>
                )}
            ...
            <div className="absolute bottom-4 left-4 right-4 text-center px-2">
                <p
                    className={cn(
                        "font-permanent-marker leading-tight",
                        (status === 'done' && memory) ? 'text-base' : 'text-lg',
                        status === 'done' && imageUrl ? 'text-black' : 'text-neutral-800'
                    )}
                    style={{ textWrap: 'balance' }}
                    >
                    {(status === 'done' && memory) ? `"${memory}"` : caption}
                </p>
            </div>
        </>
    );
