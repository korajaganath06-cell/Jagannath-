 ...
 * @param imageData A record mapping decade strings to their image data and memory.
 * @returns A promise that resolves to a data URL of the generated album page (JPEG format).
 */
export async function createAlbumPage(imageData: Record<string, { url: string, memory: string }>): Promise<string> {
    ...
    const loadedImages = await Promise.all(
        Object.values(imageData).map(data => loadImage(data.url))
    );

    const imagesWithData = decades.map((decade, index) => ({
        decade,
        img: loadedImages[index],
        memory: imageData[decade].memory,
    }));
    ...
    const reversedImages = [...imagesWithData].reverse();
    reversedImages.forEach(({ decade, img, memory }, reversedIndex) => {
        // Calculate the original index to determine grid position
        const index = imagesWithData.length - 1 - reversedIndex;
        ...
        const captionPadding = 25;
        const startY = captionAreaTop + captionPadding;
        const captionMaxWidth = polaroidWidth * 0.9;
        
        // Draw Decade
        ctx.fillStyle = '#222';
        ctx.font = `60px 'Permanent Marker', cursive`;
        ctx.fillText(decade, 0, startY);
        ...
        const lineHeight = 55;
        const memoryStartY = startY + 70; // Start below decade
        
        wrapText(ctx, memoryText, 0, memoryStartY, captionMaxWidth, lineHeight);
