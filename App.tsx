...
function App() {
    ...
    const handleGenerateClick = async () => {
        ...
        const imageToGenerate = await applyFilterToImage(uploadedImage, selectedFilter);
        ...
        const processDecade = async (decade: string) => {
            try {
                const prompt = `Reimagine the person in this photo in the style of the ${decade}. This includes clothing, hairstyle, photo quality, and the overall aesthetic of that decade. The output must be a photorealistic image showing the person clearly. Also, generate a short, nostalgic, one-sentence memory from a first-person perspective that fits the image and the time period.`;
                const { url, memory } = await generateDecadeImage(imageToGenerate, prompt);
                setGeneratedImages(prev => ({
                    ...prev,
                    [decade]: { status: 'done', url, memory },
                }));
            } catch (err) {
        ...
        try {
            const imageData = Object.entries(generatedImages)
                ...
                .reduce((acc, [decade, image]) => {
                    acc[decade] = { url: image.url!, memory: image.memory || `A fond memory from the ${decade}.` };
                    return acc;
                }, {} as Record<string, { url: string; memory: string }>);
                ...
                {appState === 'image-uploaded' && uploadedImage && (
                    <div className="flex flex-col items-center gap-6">
                         <PolaroidCard 
                            ...
                            imageFilter={FILTER_CSS[selectedFilter]}
                         />

                         <div className="flex flex-col items-center gap-3 mt-2">
                            <p className="font-permanent-marker text-neutral-400 text-lg">Apply a filter</p>
                            <div className="flex items-center gap-3">
                                {Object.entries(FILTERS).map(([key, name]) => (
                                    <button
                                        key={key}
                                        onClick={() => setSelectedFilter(key)}
                                        className={`font-permanent-marker text-sm text-center py-2 px-4 rounded-sm transform transition-all duration-200 hover:scale-110 ${
                                            selectedFilter === key 
                                            ? 'bg-yellow-400 text-black shadow-[1px_1px_0px_1px_rgba(0,0,0,0.2)]' 
                                ...
                                {DECADES.map((decade) => (
                                    <div key={decade} className="flex justify-center">
                                         <PolaroidCard
                                            ...
                                            memory={generatedImages[decade]?.memory}
